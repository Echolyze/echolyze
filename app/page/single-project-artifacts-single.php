...
.fragment-optoins-relationship .select2-container {
	padding-left: 0px;
	padding-right: 0px;
}
.svt-label {
										...
										<div id="fragmentModalOptions" style="z-index: 10; margin-top: 15px;" class="box box-default">
											...
											<form id="addFragmentForm" class="form-horizontal">
												...
												<div id="addFragmentForm_Message" class="active">
													...
													<div class="col-xs-12" style="border-bottom: 1px solid #f4f4f4">
														<div class="form-group fragment-optoins-relationship" style="margin-left: 0px; margin-right: 0px;">
															<label for="" class="col-sm-12 alpha omega">Relationship</label>
															<label for="fragmentModalOptions_NodeA" class="col-sm-3 svt-label alpha omega">Source:</label>
															<select class="form-control select2 col-xs-8" style="width: 75%;" id="fragmentModalOptions_NodeA"></select>
															<label for="fragmentModalOptions_NodeA" class="col-sm-3 svt-label alpha omega">Valence:</label>
															<select class="form-control select2 col-xs-8" style="width: 75%;" id="fragmentModalOptions_Feelings">
																<option value="positve">Is postive towards...</option>
																<option value="negative">Is negatives towards...</option>
															</select>
															<label for="fragmentModalOptions_NodeB" class="col-sm-3 svt-label alpha omega">Target:</label>
															<select class="form-control select2 col-xs-8" style="width: 75%;" id="fragmentModalOptions_NodeB"></select>
														</div>
													</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <p>Please do not navigate away from this page until saving is complete. Once complete, you will be redirected.</p>
		<div class="progress">
...
<script type="text/javascript">
	...
	function updateCompletedStepsByOne() {
		...
		if (progressPercent == 100) {
			var redirectMaybe = setInterval(completedSaveRedirectToArtifacts, 1000);
		}
	}
	function completedSaveRedirectToArtifacts() {
		if(savingCompletedSteps == savingTotalSteps) {
			window.location.replace('/app/project/?projectID=' + currentProject + '&destination=artifacts');
			return;
		}
		return;
	}
		...
		if (artifactID) {
			$.put("/api/standard.php/artifacts/" + artifactID, formData, function(result) {
				if(!isNaN(result)) {
					...
					// Deciding to go save fragments here so that we've set the artifact ID global...
					// Note: This must be done here, and thus done below aswell, as if not we won't have an artifactID to associate the fragments with...
			...
			$.post("/api/standard.php/artifacts", formData, function(result) {
				if(!isNaN(result)) {
					console.log(result);
					artifactIDGlobal = result;
					console.log("SUCCESS: Artifact Saved");
					savingExecutionHistory.append('<li>All raw artifact data saved successfully.</li>');
					// Deciding to go save fragments here so that we've set the artifact ID global...
					// Note: This must be done here, and thus done above aswell, as if not we won't have an artifactID to associate the fragments with...
					cleanFragmentsAndSaveNewIndividualFragments(allFragmentsInBody);
					updateCompletedStepsByOne();
				} else {
					console.log('Error saving artifact.');
	...
	function saveSingleFragmentToDB(fragmentElement) {
		var fragmentElementFormData = {
			related_artifact: artifactIDGlobal,
			related_project: currentProject,
