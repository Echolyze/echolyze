...
<script type="text/javascript">
	...
	function insertNewNodeRow(id, name, description, parentNode) {
		...
		newNodeToAdd.find('.delete-node-btn').click(function(e){
	...
	function deleteChildNodes(id) {
		$("[data-parent-node-id='" + id + "']").each(function() {
			var childNodeIDToDelete = $(this).attr('data-node-id');
			$.delete("/api/standard.php/nodes/" + childNodeIDToDelete,function(result) {
				if(!isNaN(result)) {
					allNodesTable.find("[data-node-id='" + childNodeIDToDelete + "']").remove();
				} else {
					console.log('Error deleting node.');
				}
			});
